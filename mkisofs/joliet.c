...
static int generate_joliet_path_tables()
{
  ...
  {
#ifdef	USE_LIBSCHILY
      comerrno(EX_BAD, "Unable to generate sane path tables - too many directories (%d)\n",
	      next_jpath_index);
#else
  ...
  for(j=1; j<next_jpath_index; j++)
  {
       ...
       {
#ifdef	USE_LIBSCHILY
	    comerrno(EX_BAD, "Fatal Joliet goof - directory has amnesia\n"); 
#else
	    fprintf(stderr,"Fatal Joliet goof - directory has amnesia\n"); 
	    exit(1);
#endif
       }
  ...
  {
#ifdef	USE_LIBSCHILY
       errmsgno(EX_BAD, "Joliet path table lengths do not match %d expected: %d\n",
	       jpath_table_index,
	       jpath_table_size);
#else
       fprintf(stderr, "Joliet path table lengths do not match %d expected: %d\n",
	       jpath_table_index,
	       jpath_table_size);
#endif
  }
...
static void FDECL2(generate_one_joliet_directory, struct directory *, dpnt, FILE *, outfile)
{
     ...
     while(s_entry) 
     {
		...
		continue;
		 ...
		 if( s_entry1 == NULL )
		 {
		      ...
		      */
#ifdef	USE_LIBSCHILY
		     comerrno(EX_BAD, "Unable to locate relocated directory\n");
#else
     ...
     {
#ifdef	USE_LIBSCHILY
	 errmsgno(EX_BAD, "Unexpected joliet directory length %d expected: %d '%s'\n",dpnt->jsize,
		 dir_index, dpnt->de_name);
#else
	 fprintf(stderr, "Unexpected joliet directory length %d expected: %d '%s'\n",dpnt->jsize,
		 dir_index, dpnt->de_name);
#endif
     }
  ...
  for(s_entry = this_dir->contents; s_entry; s_entry = s_entry->next)
      ...
      if(s_entry->isorec.flags[0] ==  2)
	{
	  if (strcmp(s_entry->name,".") != 0 && strcmp(s_entry->name,"..") != 0)
	    {
	      jpath_table_size += joliet_strlen(s_entry->name) + offsetof(struct iso_path_table, name[0]);
      ...
      if (strcmp(s_entry->name,".") != 0 && strcmp(s_entry->name,"..") != 0) 
	{
	  s_entry->jreclen = 	offsetof(struct iso_directory_record, name[0])
...
static int FDECL2(joliet_compare_dirs, const void *, rr, const void *, ll) 
{
     ...
     /*
      * If the entries are the same, this is an error. Joliet specs
      * allow for a maximum of 64 characters.
      */
     if( strncmp(rpnt, lpnt, 64) == 0 )
       {
#ifdef	USE_LIBSCHILY
	 errmsgno(EX_BAD, "Error: %s and %s have the same Joliet name\n",
		(*r)->whole_name, (*l)->whole_name);
#else
	 fprintf (stderr, "Error: %s and %s have the same Joliet name\n",
		(*r)->whole_name, (*l)->whole_name);
#endif
	 sort_goof++;
       }
...
{
     jroot_record.length[0] = 1 + offsetof(struct iso_directory_record, name[0]);
