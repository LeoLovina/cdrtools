	   ...
	   int, dirflag)
{
  ...
  while(*pnt)
	 ...
	 */
	/*
	 *  Multiple '.' characters will be allowed if we have
	 *  the "untranslated" option turned on.
	 *  davids@kithrup.com
	 */
      if(full_iso9660_filenames) 
	{
	  /* Here we allow a more relaxed syntax. */
	  if(*pnt == '.' && !allow_untranslated)
	    {
	      if (seen_dot) 
		{
		  ignore++; 
	  ...
	  if(current_length < 30) 
		...
		{
		  *result++ = ((islower((unsigned char)*pnt) && !allow_untranslated) ? toupper((unsigned char)*pnt) : *pnt);
		}
	    }
	}
      else
