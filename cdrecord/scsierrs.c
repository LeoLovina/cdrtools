...
EXPORT void
scsierrmsg(sense, status, sense_code, errvec)
...
#define	ext_sense	((struct scsi_ext_sense* ) sense)
	...
	if (status->chk == 0) {
		...
		sensemsg = scsisensemsg(dev, code, qual, errvec, sbuf);
		if (sensemsg == NULL) {
			sensemsg = scsisensemsg(dev, code, 0, errvec, sbuf);
			badqual = 1;
		}
	}
/*
